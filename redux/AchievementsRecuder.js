...
export default function achievementsReducer(state = initialState, action) {
        ...
        switch(action.type) {
            ...
            case "login": {
                localStorage.setItem("accessToken", action.userData.accessToken)
                //console.log(action.userData.refreshToken)
                document.cookie = `refreshToken=${action.userData.refreshToken}`
                return {...state,
                    name: action.userData.user.name,
                    authorisate: true,
                    id:action.userData.id,
                    password: "",
                    requirements: action.userData.user.requirements,
                    achievements: action.userData.user.achievements,
                    progress: action.userData.user.progress
                }
            }
            case "loadData": {
                return {...state,
            ...
            case "logout": {
                ...
                return {...state, name: "", authorisate:false, id: null,}
            }
        }
        if(newAchievements) {
            return {
...
function inputProgress(progress, inputs) {
    ...
    newProgress = newProgress.filter(element => 
        !(element.achievement === inputs.achievement
...
function deleteCategory(achievements, reqiurements, progress, name) {
    ...
    let newProgress = copyObjectArray(progress)
    newReqiurements = newReqiurements.filter(reqiurement => 
        reqiurement.achievement !== name)
...
function addLevel(requirements, achievement, level, newRequirements) {
    ...
    newRequirements = newRequirements.map(reqiurement => {
        return {level: level,
            achievement:achievement,
            name:reqiurement.name,
            value: reqiurement.value}
        }
    )
...
function deleteLevel(requirements, progress, level, achievement, shuffle = true) {
    ...
    newReqiurements = newReqiurements.filter(requirement =>
    ...
    if(shuffle) {
        newReqiurements = newReqiurements.map(requirement =>
            requirement.level > level ? 
            {...requirement,
