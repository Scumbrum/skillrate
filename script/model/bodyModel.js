...
export function leveler(handlers, achieve, progress, requirements, achievements) {
    let newReqiurements = requirements.filter(requirement =>
        requirement.achievement === achieve
    )
    ...
    let prevLevel = achievements.find(achievement =>
        achievement.name === achieve
    ).level
...
export function getLevels(requirements) {
    ...
    requirements.forEach(requierement => {
        ...
        if(!exist) {
            levels.push({number:requierement.level,
                requierements:[{name:requierement.name,
                                value:requierement.value
                                }
                            ]
                }
            )
            ...
            exist.requierements.push({name:requierement.name,
                                    value:requierement.value}
                                    )
        }
    });
...
function getNextRequire(achievements, requirements, achievement) {
    ...
    return requirements.filter(requirement =>
        ...
        && requirement.level === curr.level + 1)
}
...
export function generateProgressInput(handler, achievements, requirements, progress, achievement) {
    ...
    return nextRequire.map((requirement, index) =>
    <li key = {index}>{requirement.name}:
        <input
            type = "number"
            value = {values[index]}
            className = "box_input"
            onChange = {(e) => inputHandler(e.target.value, requirement.name, achievement, handler)}
        />
    </li>)
}

function getValue(achievement, progress, name) {
    const suit = progress.find(element => 
...
export function generateLevel(requirements, name, achievements, handlers) {

    let newRequirements = requirements.filter((requirement) =>
    requirement.achievement===name)
        ...
        <div className="level_head">
            ...
            <div className="level_controlls">
                <FontAwesomeIcon icon= {faPen} onClick={() => 
                    handlers[0](levelFilter(newRequirements, level), level.number)}
                    className="level_edit"/>
                <FontAwesomeIcon 
                icon= {faTimes} 
                onClick={() => deleteHandler(handlers[1], level.number, requirements, name)}
                className="level_edit"/>
            </div>: null}
        </div>
        <div className="level_body">
            <ol>
                {level.requierements.map((requirement, index) =>
                <li key={index}>
                    {requirement.name} - {requirement.value}
                </li>)}
            </ol>
...
function levelFilter(reqiurements, level) {
    const newReqiurements = copyObjectArray(reqiurements)
    return newReqiurements.filter(requirement => requirement.level === level.number)
}
