...
export function generateInputs(inputs, handlers){
    ...
    <li key = {index}>
        ...
        <input className = "frame_input-value" type = "number" value={input.value} onChange={(e) => handlers[1](index,  e.target.value >= 0 ? parseInt(e.target.value) : 0)}/>
        <FontAwesomeIcon className="frame_delete" icon={faTimes} onClick={() => handlers[2](index)}/>
    </li>)
}

export function logoutHandler(handler) {
    const cookie = parseCookie(document.cookie)
    handler(cookie.refreshToken)
}
